...
async def load_textbook(session, json_path: Path) -> tuple[int, int]:
    """Load one textbook JSON file. Returns (chunks_added, chunks_skipped)."""
    try:
    ...
    return added, len(chunks_data) - added
    ...
    async with AsyncSessionLocal() as session:
        for board_dir in EXTRACTED_DIR.iterdir():
            ...
            for json_file in board_dir.glob("*.json"):
                total_files += 1
                added, _ = await load_textbook(session, json_file)
                total_added += added
                if added > 0:
