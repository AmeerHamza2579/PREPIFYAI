...
# Load app/.env into os.environ so GROQ_API_KEY etc. are available
try:
...
async def lifespan(app: FastAPI):
    # Preload FAISS + SentenceTransformer in background so first /generate-questions/ is fast
    def _warmup():
        try:
...
def create_application() -> FastAPI:
    ...
    async def connection_refused_handler(request: Request, exc: Exception):
        if _is_connection_refused(exc):
            return JSONResponse(
                ...
                content={
                    "detail": (
                        "Database is unreachable. Start PostgreSQL (e.g. run: docker-compose up -d) "
                        "and ensure DATABASE_URL in app/.env is correct (e.g. localhost:5433)."
                    )
                },
            )
        raise exc
    ...
    async def operational_error_handler(request: Request, exc: OperationalError):
        logging.getLogger("prepifyai.api").exception("OperationalError on %s", request.url.path)
        return JSONResponse(
            status_code=503,
            content={
                "detail": (
                    "Cannot reach PostgreSQL. Start Docker (docker compose up -d), run "
                    "alembic upgrade head from FYP-Backend-main, and check DATABASE_URL in app/.env "
                    "(often localhost:5433 for this project)."
                )
            },
        )
    ...
    async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError):
        ...
        detail = "A database error occurred. See the API terminal log for the full traceback."
        if settings.DEBUG_API:
            orig = getattr(exc, "orig", None)
            tail = repr(orig) if orig is not None else repr(exc)
            detail = f"{type(exc).__name__}: {tail}"
        return JSONResponse(
