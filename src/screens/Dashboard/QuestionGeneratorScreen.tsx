...
export default function QuestionGeneratorScreen() {
  ...
  const generateQuestions = async () => {
    ...
    setLoading(true);
    try {
      const { questions: list, retrieval_sources } = await questionService.generateQuestions({
      ...
      setQuestions(list);
      setRetrievalSources(retrieval_sources);
      if (list.length === 0) {
        Alert.alert('No questions', 'Try another topic or check API / GROQ_API_KEY on the server.');
      }
    } catch (e: unknown) {
