...
class QuestionGenerationService:
    ...
    async def _get_subject_by_name(self, subject_name: str) -> Optional[Subject]:
        """Get subject by name (case-insensitive search)"""
        # Normalize subject name (remove extra spaces, title case)
        normalized_name = subject_name.strip().title()
        
        # Try exact match first
        result = await self.db.execute(
            select(Subject).where(Subject.subject_name.ilike(normalized_name)).limit(1)
        )
        ...
        if not subject:
            result = await self.db.execute(
                select(Subject).where(Subject.subject_name.ilike(f"%{normalized_name}%")).limit(1)
            )
            subject = result.scalar_one_or_none()
        
        # If still no match and multiple subjects exist with similar names, 
        # log a warning and return the first one
        if not subject:
            result = await self.db.execute(
                select(Subject).where(Subject.subject_name.ilike(f"%{normalized_name}%"))
            )
            subjects = result.scalars().all()
            if subjects:
                logger.warning(f"Multiple subjects found matching '{subject_name}'. Using first match: {subjects[0].subject_id} - {subjects[0].subject_name} ({subjects[0].board}, Class {subjects[0].class_level})")
                return subjects[0]
    ...
    async def _get_subject_by_name_and_board(
        ...
        """Get subject by name, board, and class level for precise matching"""
        # Normalize inputs
        normalized_name = subject_name.strip().title()
        normalized_board = board_name.strip().upper() if board_name else None
        
        # Normalize class_level (handle "9", "class9", "Class 9", etc.)
        normalized_class = None
        if class_level:
            class_str = str(class_level).strip().lower()
            # Extract number from strings like "class9", "Class 9", "9", etc.
            match = re.search(r'\d+', class_str)
            if match:
                normalized_class = match.group(0)
        
        # Build query with filters
        conditions = [Subject.subject_name.ilike(normalized_name)]
        
        if normalized_board:
            conditions.append(Subject.board.ilike(normalized_board))
        
        if normalized_class:
            # Match class_level as string (could be "9", "10", etc.)
            conditions.append(
                or_(
        ...
        # First try to find a subject with chunks (prefer subjects with content)
        result = await self.db.execute(
            select(Subject)
            .where(and_(*conditions))
            .outerjoin(TextbookChunk, Subject.subject_id == TextbookChunk.subject_id)
            .group_by(Subject.subject_id)
            .having(func.count(TextbookChunk.chunk_id) > 0)
            .limit(1)
        )
        subject = result.scalar_one_or_none()
        
        # If no subject with chunks found, try without the chunk requirement
        if not subject:
            result = await self.db.execute(
                select(Subject).where(and_(*conditions)).limit(1)
            )
            subject = result.scalar_one_or_none()
        
        if not subject and normalized_board:
            # Try without class_level if board is specified
            conditions_without_class = [Subject.subject_name.ilike(normalized_name)]
            conditions_without_class.append(Subject.board.ilike(normalized_board))
            result = await self.db.execute(
                select(Subject).where(and_(*conditions_without_class)).limit(1)
            )
            subject = result.scalar_one_or_none()
