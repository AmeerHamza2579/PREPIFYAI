  ...
  const generatePaper = async () => {
    ...
    try {
      ...
      const [mcqRes, shortRes, longRes] = await Promise.all([
        questionService.generateQuestions({ ...base, qtype: 'MCQ', num_questions: 8 }),
        questionService.generateQuestions({ ...base, qtype: 'Short', num_questions: 5 }),
        questionService.generateQuestions({ ...base, qtype: 'Long', num_questions: 3 }),
      ]);
      const merged = mergeRetrievalSources(
        mcqRes.retrieval_sources,
        shortRes.retrieval_sources,
        longRes.retrieval_sources
      );
      setRetrievalSources(merged);
      setPaper([
        { type: 'mcq', questions: mcqRes.questions },
        { type: 'short', questions: shortRes.questions },
        { type: 'long', questions: longRes.questions },
      ]);
    } catch (e: unknown) {
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          <View style={styles.panel}>
            ...
            <Text style={styles.note}>
              Runs three Groq generations in parallel (MCQ, short, long). Ensure GROQ_API_KEY is set on
              the server.
            </Text>
          </View>
