...
export default function QuestionPracticeCard({
  ...
  const parsed = useMemo(() => parseMcqFromText(item.question), [item.question]);

  const correctLetter = useMemo(

    () => (parsed ? extractMcqCorrectLetter(item.answer, parsed.options) : null),
  ...
  const useMcqTap =
    ...
    presentation === 'mcq' && parsed !== null && correctLetter !== null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        /* Groq may be unset; user can still tap “Get detailed explanation”. */
          ...
          {parsed.options.map((opt) => {
            ...
            return (
              ...
              <TouchableOpacity
                ...
                accessibilityLabel={`Option ${opt.letter}: ${opt.text}`}

              >

                <Text style={styles.mcqOptionLetter}>{opt.letter}.</Text>

                <Text style={styles.mcqOptionText}>{opt.text}</Text>
